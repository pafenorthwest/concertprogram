...
export async function lookupByDetails(
	...
	try {
		...
		const searchQuery =
			...
			performerLastName +
			"' \n" +
			"  AND (LOWER(composer.full_name) LIKE '%" +
			composer.toLowerCase() +
			"' OR LOWER(composer.notes) = '" +
			composer.toLowerCase() +
			"') \n" +
			'  AND performer.epoch = ' +
			birthYear +
			' \n' +
			'  AND performance.pafe_series =' +
			pafe_series() +
			' \n' +
...
export async function insertTable(
	table: string,
	data: ComposerInterface | AccompanistInterface | MusicalPieceInterface | PerformerInterface
) {
	try {
		const connection = await pool.connect();
		...
		switch (table) {
			case 'composer':
				...
				inputVals =
					"('" +
					(data as ComposerInterface).full_name.replaceAll("'", "''").trim() +
					"', '" +
					(data as ComposerInterface).years_active +
					"')";
				// add notes
				if (isNonEmptyString((data as ComposerInterface).notes)) {
					...
					inputVals =
						inputVals.slice(0, -1) +
						", '" +
						(data as ComposerInterface).notes.replaceAll("'", "''").trim() +
						"')";
				}
				// return id
				break;
	...
	try {
		...
		switch (table) {
			case 'composer':
				...
				if (
					!(
						isNonEmptyString((data as ComposerInterface).full_name) &&
...
export async function queryPerformances(filters?: PerformanceFilterInterface) {
	...
	try {
		...
		const order =
			'ORDER BY performance.pafe_series, performance.concert_series, performance.performance_order';
...
export async function searchComposer(composer_name: string) {
	try {
		...
		const searchSQL =
			'SELECT id, full_name, years_active, notes ' +
			'FROM composer ' +
			"WHERE full_name = '" +
			composer_name +
			"' OR LOWER(notes) = '" +
...
export async function selectDBSchedule(performer_id: number) {
	try {
		...
		const selectSQL =
			'SELECT performer_id, concert_series, pafe_series, \n' +
