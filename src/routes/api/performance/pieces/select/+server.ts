...
import { selectPerformancePiece } from '$lib/server/db';
...
export async function POST({ url, request, cookies }) {
	...
	const updated = await selectPerformancePiece(performanceId, musicalPieceId);
	if (updated === 0) {
		return json({ status: 'error', reason: 'Performance piece not found' }, { status: 404 });
	}
