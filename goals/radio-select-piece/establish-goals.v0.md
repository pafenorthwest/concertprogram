...
1. The approved `goals-extract.sh` and `goals-validate.sh` scripts currently mis-parse normal `## Goals` / `## Success criteria` sections, so the mandatory validation step fails even though verifiable goals are present.
...
1. Do you want the ACAC tooling bug fixed first so this task can proceed within the required lifecycle?
2. Once that blocker is removed, do these drafted goals capture the intended rule that any schedule context with more than one candidate piece must require a performer choice before submit?
...
- Stop with `BLOCKED` until the mandatory establish-goals validation tooling can validate a non-blocked goals artifact.
