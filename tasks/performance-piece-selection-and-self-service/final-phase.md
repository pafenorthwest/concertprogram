...
* [ ] `/docs` audit and updates

  * [ ] Enumerate documentation artifacts under `/docs` that are impacted by this change (API behavior, auth, error contracts, examples, migrations, ops notes).
  * [ ] Update affected docs and ensure cross-links remain valid (README ↔ docs ↔ ADRs/runbooks).

* [ ] YAML documentation contracts (`/docs/**/*.yaml`)

  * [ ] Discover all YAML documentation files under `/docs` (recursive) and update any impacted by the change:
  ...
  * [ ] If a YAML contract/spec is required for this change but no matching file exists yet:
    ...
    * [ ] Create an initial YAML spec in `/docs/` using the repo’s conventions (OpenAPI/AsyncAPI/etc. as applicable)
...
* [ ] README updates
  ...
  * [ ] Add or refresh a single “Documentation” section linking to `/docs/` and any key YAML specs within it.
...
- [ ] Coverage gaps:
- [ ] Notes: `npm run test` emits existing warnings about missing performance details in program tests.
...
- Severity: Low
- Repro: `npm run build` prints SvelteKit adapter warnings about missing optional deps (canvas/pg-native) and deprecated `csrf.checkOrigin`.
- Suggested fix: Track upstream config/deps; not introduced by this change.
